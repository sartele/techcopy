...
if (!can_manage_consumables())              json_response(['error'=>'Non autorizzato'], 403);

$data  = json_decode(file_get_contents('php://input'), true);
$id    = (int)($data['id']   ?? 0);
$delta = (int)($data['delta'] ?? 0);
if (!$id || $delta === 0) json_response(['error'=>'Parametri non validi'], 400);

$db = db();
$db->prepare("UPDATE consumables SET stock=GREATEST(0,stock+?) WHERE id=?")->execute([$delta,$id]);
$row = $db->prepare("SELECT stock,min_stock,unit FROM consumables WHERE id=?");
...
json_response(['success'=>true,'stock'=>$row['stock'],'unit'=>$row['unit'],'pct'=>round($pct)]);

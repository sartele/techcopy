...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($action==='create') {
        ...
        if ($exists->fetch()) { flash('Email già in uso.','error'); redirect('/techcopy/pages/users.php'); }
        $hash = password_hash($_POST['password'], PASSWORD_DEFAULT);
        $initials = strtoupper(implode('',array_map(fn($w)=>$w[0], explode(' ',trim($_POST['name'])))));
        $initials = substr($initials,0,2);
        $colors = ['#00d4ff','#a78bfa','#39d353','#f79000','#f44747','#e879f9'];
    ...
    if ($action==='toggle') {
        $id=(int)$_POST['id'];
        if ($id==current_user()['id']) { flash('Non puoi disattivare te stesso.','error'); redirect('/techcopy/pages/users.php'); }
    ...
    if ($action==='change_role') {
        $id=(int)$_POST['id'];
        if ($id==current_user()['id']) { flash('Non puoi cambiare il tuo ruolo.','error'); redirect('/techcopy/pages/users.php'); }
        $db->prepare("UPDATE users SET role=? WHERE id=?")->execute([$_POST['role'],$id]);
        flash('Ruolo aggiornato ✅'); redirect('/techcopy/pages/users.php');
    }
}

$users = $db->query("SELECT u.*,
    (SELECT COUNT(*) FROM tickets t WHERE t.tech_id=u.id AND t.status!='closed') AS open_tickets,
    (SELECT COUNT(*) FROM tickets t WHERE t.tech_id=u.id) AS total_tickets
    FROM users u ORDER BY u.active DESC, FIELD(u.role,'admin','supervisor','tech','viewer'), u.name")->fetchAll();
  ...
  <div style="padding:24px">
    <form method="POST">
      ...
      <div class="form-grid">
        ...
        <div class="form-group"><label>Email *</label><input type="email" name="email" placeholder="mario@techcopy.it" required></div>
  ...
  <table>
    ...
    <tbody>
      ...
      <tr style="<?= !$u['active']?'opacity:.5':'' ?>">
        <td>
          <div style="display:flex;align-items:center;gap:10px">
            ...
            <?php if ($u['id']==current_user()['id']): ?><span class="chip" style="font-size:10px">Tu</span><?php endif; ?>
          </div>
        </td>
      ...
      <?= csrf_field() ?>
          ...
          <?php else: ?>
            <span class="badge badge-<?= h($u['role']) ?>"><?= h(match($u['role']){'admin'=>'Admin','supervisor'=>'Supervisore','tech'=>'Tecnico','viewer'=>'Viewer',default=>$u['role']}) ?></span>
          <?php endif; ?>
        </td>
...
<div id="pwd-modal" style="display:none" class="modal-overlay" onclick="if(event.target===this)this.style.display='none'">
  <div class="modal" style="width:400px">
    <div class="modal-header">
      ...
      <button class="close-btn" onclick="document.getElementById('pwd-modal').style.display='none'">✕</button>
    </div>
    <div class="modal-body">
      <form method="POST" id="pwd-form">
      <?= csrf_field() ?>
        <input type="hidden" name="action" value="reset_pwd">
        <input type="hidden" name="id" id="pwd-user-id">
        <div class="form-group">
          <label>Nuova password per <strong id="pwd-user-name"></strong></label>
          <input type="password" name="new_password" placeholder="Minimo 6 caratteri" minlength="6" required>
        </div>

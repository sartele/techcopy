...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($waitSec > 0) {
        ...
        // (un attaccante non può distinguere "utente non esiste" da "password sbagliata"
        //  in base al tempo di risposta)
        $start = microtime(true);

        if (login_user(trim($_POST['email'] ?? ''), $_POST['password'] ?? '')) {
            // Redirect alla pagina originale richiesta (se presente e sicura)
            $next = $_GET['next'] ?? '';
        ...
        // Pausa minima 500ms per evitare timing attack / brute force rapido
...
<?php
// Header sicurezza anche sulla pagina di login
...
<div class="login-screen">
  <div class="login-box">
    ...
    <?php
    // ── ACCOUNT DEMO ─────────────────────────────────────────
    // IMPORTANTE: In produzione impostare APP_DEBUG=false in config.php
    // per nascondere questa sezione con le credenziali di esempio.

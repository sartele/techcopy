...
if ($_SERVER['REQUEST_METHOD']==='POST') {
    ...
    if ($action==='edit_user') {
        ...
        if ($name === '' || $email === '') {
            ...
            redirect('/techcopy/pages/users.php?edit='.$id);
        }

        // Controllo duplicati email (esclude l'utente stesso)
        ...
        $allowedRoles = ['admin','supervisor','tech','viewer','insert'];
        $role = $_POST['role'] ?? '';

        if ($id === $me['id']) {
            // Non permettere di auto-modificare il proprio ruolo da questo form
        ...
        // Ricalcola le iniziali avatar in base al nuovo nome
        $words = explode(' ', $name);
        $initials = strtoupper(implode('', array_map(fn($w)=>$w[0], $words)));
        $initials = substr($initials,0,2);
        $db->prepare("UPDATE users SET avatar=? WHERE id=?")->execute([$initials, $id]);
...
// Carica l'utente da modificare (pannello inline)
...
<div class="table-wrap" style="margin-bottom:24px;padding:0;border-color:var(--accent)">
  <div class="table-head" style="border-bottom-color:var(--accent)">
    <h4 style="color:var(--accent)">✏️ Modifica: <?= h($editUser['name']) ?></h4>
    <a href="/techcopy/pages/users.php" class="btn btn-secondary btn-sm">✕ Annulla</a>
  </div>
  <div style="padding:24px">
    <form method="POST">
      <?= csrf_field() ?>
      ...
      <div class="form-grid">
        <div class="form-group">
          ...
          <input name="name" value="<?= h($editUser['name']) ?>" required>
        </div>
        <div class="form-group">
          <label>Email *</label>
        ...
        <div class="form-group">
          ...
          <input type="text" value="<?= h(($roleMeta[$editUser['role']][0] ?? '•').' '.($roleMeta[$editUser['role']][2] ?? $editUser['role'])) ?>" disabled>
          <div style="font-size:11px;color:var(--text3);margin-top:4px">Non puoi modificare il tuo ruolo da qui.</div>
          <?php else: ?>
          <select name="role">
            <option value="admin"      <?= $editUser['role']==='admin'?'selected':'' ?>>👑 Amministratore</option>
            <option value="supervisor" <?= $editUser['role']==='supervisor'?'selected':'' ?>>🔍 Supervisore</option>
            <option value="tech"       <?= $editUser['role']==='tech'?'selected':'' ?>>🔧 Tecnico</option>
            <option value="viewer"     <?= $editUser['role']==='viewer'?'selected':'' ?>>👁 Visualizzatore</option>
            <option value="insert"     <?= $editUser['role']==='insert'?'selected':'' ?>>🔧 Inserimento</option>
          </select>
          <?php endif; ?>
        </div>
      </div>
      <div style="display:flex;gap:10px;flex-wrap:wrap">
        ...
        <a href="/techcopy/pages/users.php" class="btn btn-secondary">Annulla</a>
      </div>
    </form>
  </div>
</div>
  ...
  <table>
    ...
    <tbody>
      <?php foreach($users as $u):
        ...
        $isEditing = ($editId === (int)$u['id']);
      ?>
      <tr style="<?= !$u['active']?'opacity:.5;':'' ?><?= $isEditing?'background:var(--accent-dim);':'' ?>">
        <td>
        ...
        <td>
          <div class="td-actions">
            <a href="?edit=<?= $u['id'] ?>"
               ...
               title="Modifica dati utente">✏️ Modifica</a>

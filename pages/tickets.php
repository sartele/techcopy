...
if (isset($_GET['id'])) {
    ...
    if ($detailTicket) {
        ...
        $su = $db->prepare("
  SELECT tu.*, u.name AS user_name, u.avatar, u.color, u.role 
  FROM ticket_users tu 
  JOIN users u ON u.id=tu.user_id 
  WHERE tu.ticket_id=? 
  ORDER BY 
    (tu.role_note = 'Responsabile') DESC,
    tu.added_at ASC
"); $su->execute([$detailTicket['id']]); $detailTicket['assigned_users']=$su->fetchAll();
    }
}
...
$techUsers = $db->query("SELECT id,name,role,color,avatar FROM users WHERE role IN ('supervisor','tech') AND active=1 ORDER BY FIELD(role,'supervisor','tech'),name")->fetchAll();
...
<!-- Codice per dividere il Responsabile dell'assistenza dal resto del team -->
<?php
$responsabile = null;
$team = [];

if (!empty($t['assigned_users'])) {
    foreach ($t['assigned_users'] as $au) {
        $role = trim($au['role_note'] ?? '');
...
<div class="desktop-only-header">
  <div style="margin-bottom:16px;display:flex;gap:8px;flex-wrap:wrap">
	...
	<a href="/techcopy/pages/report_ticket_compact.php?id=<?= $t['id'] ?>" class="btn btn-secondary btn-sm">🖨 Report Compatto</a>
  </div>
...
<div class="mobile-back-bar">
  <a href="/techcopy/pages/tickets.php" class="btn btn-secondary btn-sm">← Lista</a>
  <a href="/techcopy/pages/report_ticket.php?id=<?= $t['id'] ?>" target="_blank" class="btn btn-secondary btn-sm">📄 Rapporto</a>
  <a href="/techcopy/pages/report_ticket_compact.php?id=<?= $t['id'] ?>" target="_blank" class="btn btn-secondary btn-sm">🖨 Report Compatto</a>
</div>

<div class="ticket-hero-card">
  <div class="ticket-hero-top">
    <span class="ticket-id" style="font-size:15px">#<?= str_pad($t['id'],4,'0',STR_PAD_LEFT) ?></span>
    <?= status_badge($t['status']) ?>
    <?= priority_badge($t['priority']) ?>
    <span class="chip"><?= h($t['type']) ?></span>
  </div>
  <h2 class="ticket-hero-title"><?= h($t['title']) ?></h2>
  <div class="ticket-hero-meta">
    <span>🏢 <strong><?= h($t['client_name']) ?></strong></span>
    <span>🖨️ <?= h($t['brand'].' '.$t['model']) ?></span>
    <?php if ($t['tech_name']): ?><span style="color:<?= h($t['tech_color']??'var(--text2)') ?>">👤 <?= h($t['tech_name']) ?></span><?php endif; ?>
  </div>
  <!-- Descrizione problema sempre visibile -->
  <div class="ticket-hero-problem">
    <div class="ticket-hero-problem-label">📌 Problematica</div>
    <div class="ticket-hero-problem-text"><?= nl2br(h($t['description'])) ?></div>
  </div>
</div>

<!-- ── TAB NAVIGATION MOBILE (Lavoro per primo) ───────────────── -->
<div class="tabs tabs-mobile">
  <?php if ($canEdit): ?>
  <div class="tab active" onclick="switchTab(this,'work-edit')">🔧 Lavoro</div>
  <?php else: ?>
  <div class="tab active" onclick="switchTab(this,'work')">🔧 Lavoro</div>
  <?php endif; ?>
  <div class="tab" onclick="switchTab(this,'overview')">📋 Info</div>
  <div class="tab" onclick="switchTab(this,'history')">📜 Storico</div>
  <?php if ($canEdit): ?><div class="tab" onclick="switchTab(this,'edit')">⚙️ Avanzate</div><?php endif; ?>
</div>

<!-- ── TAB NAVIGATION DESKTOP (Panoramica per prima) ─────────── -->
<div class="tabs tabs-desktop">
  <div class="tab active" onclick="switchTab(this,'overview')">📋 Panoramica</div>
  <?php if ($canEdit): ?>
  <div class="tab" onclick="switchTab(this,'work-edit')">🔧 Lavoro</div>
  <?php else: ?>
...
<!-- ── TAB: LAVORO (con form rapido inline per tecnici) ─────── -->
<?php if ($canEdit): ?>
<div data-tab="work-edit">
  <!-- Form rapido operativo -->
  <form method="POST" enctype="multipart/form-data" id="quick-work-form">
    <?= csrf_field() ?>
    <input type="hidden" name="action" value="update">
    <input type="hidden" name="ticket_id" value="<?= $t['id'] ?>">
    <!-- Campi nascosti che passano i valori invariati -->
    <input type="hidden" name="status"        id="qf-status"  value="<?= h($t['status']) ?>">
    <input type="hidden" name="priority"      value="<?= h($t['priority']) ?>">
    <input type="hidden" name="type"          value="<?= h($t['type']) ?>">
    <input type="hidden" name="tech_id"       value="<?= h($t['tech_id'] ?? '') ?>">
    <input type="hidden" name="title"         value="<?= h($t['title']) ?>">
    <input type="hidden" name="description"   value="<?= h($t['description']) ?>">
    <input type="hidden" name="notes"         value="<?= h($t['notes'] ?? '') ?>">
    <input type="hidden" name="travel_time"   id="qf-travel"  value="<?= (int)$t['travel_time'] ?>">
    <input type="hidden" name="work_time"     id="qf-work"    value="<?= (int)$t['work_time'] ?>">
    <input type="hidden" name="counter_bw"    value="<?= (int)$t['counter_bw'] ?>">
    <input type="hidden" name="counter_color" value="<?= (int)$t['counter_color'] ?>">
    <!-- Passa gli utenti assegnati -->
    <?php foreach (($t['assigned_users'] ?? []) as $au): ?>
    <input type="hidden" name="assigned_users[]" value="<?= (int)$au['user_id'] ?>">
    <?php endforeach; ?>
    <!-- Passa i componenti esistenti -->
    <?php foreach (array_filter($t['parts'], fn($p) => !empty($p['consumable_id'])) as $p): ?>
    <input type="hidden" name="consumable_id[]" value="<?= (int)$p['consumable_id'] ?>">
    <input type="hidden" name="consumable_qty[]" value="<?= (int)$p['quantity'] ?>">
    <?php endforeach; ?>
    <?php foreach (array_filter($t['parts'], fn($p) => empty($p['consumable_id'])) as $p): ?>
    <input type="hidden" name="part_free[]" value="<?= h($p['part_name']) ?>">
    <?php endforeach; ?>

    <!-- REPORT LAVORO — campo principale -->
    <div class="qf-section">
      <label class="qf-label">📋 Resoconto attività svolta</label>
      <textarea name="work_report" class="qf-textarea" rows="6"
        placeholder="Descrivi le operazioni eseguite: interventi, sostituzioni, calibrazioni..."><?= h($t['work_report'] ?? '') ?></textarea>
    </div>

    <!-- TEMPI: editing rapido con +/- -->
    <div class="qf-times-grid">
      <div class="qf-time-box">
        <div class="qf-time-label">🚗 Trasferta (min)</div>
        <div class="qf-stepper">
          <button type="button" class="qf-step-btn" onclick="stepTime('qf-travel',-15)">−</button>
          <input type="number" id="qf-travel-input" class="qf-step-val" value="<?= (int)$t['travel_time'] ?>" min="0"
            oninput="document.getElementById('qf-travel').value=this.value">
          <button type="button" class="qf-step-btn" onclick="stepTime('qf-travel',15)">+</button>
        </div>
      </div>
      <div class="qf-time-box">
        <div class="qf-time-label">⏱️ Lavoro (min)</div>
        <div class="qf-stepper">
          <button type="button" class="qf-step-btn" onclick="stepTime('qf-work',-15)">−</button>
          <input type="number" id="qf-work-input" class="qf-step-val" value="<?= (int)$t['work_time'] ?>" min="0"
            oninput="document.getElementById('qf-work').value=this.value">
          <button type="button" class="qf-step-btn" onclick="stepTime('qf-work',15)">+</button>
        </div>
      </div>
    </div>
    ...
    <div class="qf-section">
      <label class="qf-close-label">
        <input type="checkbox" name="resolved" id="qf-resolved" <?= $t['resolved']?'checked':'' ?>
          onchange="document.getElementById('qf-close-notice').style.display=this.checked?'flex':'none'">
        <span>✅ Chiudi intervento — segna come risolto</span>
      </label>
      <div id="qf-close-notice" style="display:<?= $t['resolved']?'flex':'none' ?>;align-items:center;gap:8px;margin-top:8px;padding:10px 14px;background:var(--green-dim);border:1px solid var(--green);border-radius:var(--radius);font-size:13px;color:var(--green)">
        🟢 Al salvataggio l'assistenza verrà <strong>chiusa</strong>.
      </div>
    </div>
    <?php else: ?>
    <div style="padding:10px 14px;background:var(--green-dim);border:1px solid var(--green);border-radius:var(--radius);font-size:13px;color:var(--green);margin-bottom:16px">
      🟢 Intervento già chiuso.
    </div>
    <?php endif; ?>
	<a href="/techcopy/pages/sign_ticket.php?id=<?= $t['id'] ?>" class="btn btn-success">✍ Firma Cliente</a>
<?php if(!empty($t['client_signature_path'])): ?>
    <span class="badge badge-closed">Firmato</span>
<?php endif; ?>

    <button type="submit" class="btn btn-primary qf-save-btn">💾 Salva</button>
  </form>

  <!-- Riepilogo dati lavoro (sola lettura) -->
  <div class="divider" style="margin:28px 0 16px"></div>
  <div class="section-title">Riepilogo tempi e contatori</div>
  <div class="info-grid" style="margin-bottom:20px">
    <div class="info-box"><div class="info-label">Trasferta</div><div class="info-value" style="font-family:var(--mono)"><?= format_minutes((int)$t['travel_time']) ?></div></div>
    <div class="info-box"><div class="info-label">Lavoro</div><div class="info-value" style="font-family:var(--mono)"><?= format_minutes((int)$t['work_time']) ?></div></div>
    ...
    <div class="info-box"><div class="info-label">Contatore BN</div><div class="info-value" style="font-family:var(--mono)"><?= number_format((int)$t['counter_bw']) ?></div></div>
    <div class="info-box"><div class="info-label">Contatore Colore</div><div class="info-value" style="font-family:var(--mono)"><?= number_format((int)$t['counter_color']) ?></div></div>
  </div>
  ...
  <div style="margin-bottom:20px">
    <?php foreach($t['parts'] as $p):
      $isLinked = !empty($p['consumable_id']); $qty = (int)($p['quantity'] ?? 1); ?>
...
<!-- PANORAMICA -->
<div data-tab="overview" class="hidden">
  <div class="info-grid" style="margin-bottom:20px">
...
<!-- Gestione team con Responsabile diviso dal resto del Team -->
	<div class="info-box">
  <div class="info-label">Team assegnato</div>
  <div class="info-value">
    ...
    <?php if ($responsabile): ?>
      ...
      <div style="margin-bottom:10px">
        <div style="font-size:11px;color:var(--text3);margin-bottom:4px">👑 Responsabile</div>
        
        <div style="display:flex;align-items:center;gap:8px">
          <div style="
            width:26px;height:26px;border-radius:50%;
            background:<?= h(($responsabile['color'] ?? '#999').'22') ?>;
            color:<?= h($responsabile['color'] ?? '#999') ?>;
            border:1.5px solid <?= h($responsabile['color'] ?? '#999') ?>;
            display:flex;align-items:center;justify-content:center;
            font-size:11px;font-weight:700;
          ">
            <?= h($responsabile['avatar'] ?? strtoupper(substr($responsabile['user_name'],0,1))) ?>
          </div>

          <span style="font-size:13px;font-weight:600">
            <?= h($responsabile['user_name']) ?>
          </span>
        </div>
      </div>
    <?php endif; ?>
    ...
    <?php if (!empty($team)): ?>
      <!-- TEAM -->
      <div>
        <div style="font-size:11px;color:var(--text3);margin-bottom:4px">👥 Team</div>

        <?php foreach ($team as $au): ?>
        <div style="display:flex;align-items:center;gap:6px;margin-bottom:4px">
          <div style="
            width:22px;height:22px;border-radius:50%;
            background:<?= h(($au['color'] ?? '#999').'22') ?>;
            color:<?= h($au['color'] ?? '#999') ?>;
            border:1.5px solid <?= h($au['color'] ?? '#999') ?>;
            display:flex;align-items:center;justify-content:center;
            font-size:10px;font-weight:700;
          ">
            <?= h($au['avatar'] ?? strtoupper(substr($au['user_name'],0,1))) ?>
          </div>
    ...
    <?php if (!$responsabile && empty($team) && $t['tech_name']): ?>
      <!-- FALLBACK -->
      <div style="display:flex;align-items:center;gap:6px">
        <span style="color:<?= h($t['tech_color'] ?? 'var(--text2)') ?>">
          👤 <?= h($t['tech_name']) ?>
        </span>
      </div>
    <?php endif; ?>
...
<!-- LAVORO (view-only, for non-editors) -->
<div data-tab="work" <?= $canEdit ? 'class="hidden"' : '' ?>>
  <div class="info-grid" style="margin-bottom:20px">
      ...
      <div class="form-group form-full">
        ...
        <div style="display:flex;flex-direction:column;gap:6px;background:var(--surface2);border:1px solid var(--border);border-radius:var(--radius);padding:12px">
          ...
          <label style="display:flex;align-items:center;gap:10px;cursor:pointer;padding:6px 8px;border-radius:4px;transition:background .15s" onmouseover="this.style.background='var(--surface3)'" onmouseout="this.style.background=''">
            ...
            <div style="width:28px;height:28px;border-radius:50%;background:<?= h($u['color'] ?? '#999') ?>22;color:<?= h($u['color'] ?? '#999') ?>;border:1.5px solid <?= h($u['color'] ?? '#999') ?>;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:700;flex-shrink:0"><?= h($u['avatar'] ?? '?') ?></div>
...
// Override di switchTab: sincronizza entrambe le barre tab e mostra il pannello corretto
function switchTab(clickedTab, tabId) {
  // Attiva il tab cliccato nella sua barra e deseleziona gli altri
  const parentBar = clickedTab.closest('.tabs');
  parentBar.querySelectorAll('.tab').forEach(t => t.classList.remove('active'));
  clickedTab.classList.add('active');

  // Sincronizza l'altra barra tab (mobile ↔ desktop)
  document.querySelectorAll('.tabs').forEach(bar => {
    if (bar === parentBar) return;
    bar.querySelectorAll('.tab').forEach(t => {
      const match = t.getAttribute('onclick')?.includes("'" + tabId + "'");
...
function stepTime(hiddenId, delta) {
  const hidden = document.getElementById(hiddenId);
  const inputId = hiddenId + '-input';

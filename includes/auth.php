...
function is_insert(): bool {
    $u = current_user(); return $u && $u['role'] === 'insert';
}
...
function can_create_ticket(): bool {
    $u = current_user();
    return $u && in_array($u['role'], ['admin','supervisor','tech','insert']);
}
function can_manage_consumables(): bool {
    $u = current_user();
    return $u && in_array($u['role'], ['admin','supervisor','insert']);
}
...
function can_edit_ticket(int $techId, int $ticketId = 0): bool {
    $u = current_user();
    if (!$u) return false;
    if (in_array($u['role'], ['admin','supervisor','insert'])) return true;
    if ($u['role'] === 'tech') {
